...
# -*- coding: utf-8 -*-
"""
Testing script for Mask2Former Fusion models on ZOD weather conditions.
"""
import os
import json
import argparse
...
def _prepare_inputs_for_mode(images, lidar, mode):
    """Prepare inputs for model call based on CLI mode."""
    if mode == 'rgb':
...
def test_model_on_weather(config, model, device, weather_condition, checkpoint_path):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(val_loader, desc=f"Testing {weather_condition}")):
            ...
            # Forward pass
            # Mask2FormerFusion returns (None, segmap, all_class_logits, all_masks)
            rgb_input, lidar_input = _prepare_inputs_for_mode(images, lidar, config['CLI']['mode'])
            model_outputs = model(rgb_input, lidar_input, config['CLI']['mode'])
            outputs = model_outputs[1]  # segmap: [B, num_classes, H, W]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Mask2Former Fusion on ZOD weather conditions")
