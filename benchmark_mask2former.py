...
# -*- coding: utf-8 -*-
"""
Mask2Former Model Benchmarking Script

This script benchmarks Mask2Former models across different configurations, modalities,
...
class Mask2FormerBenchmarker:
    """Comprehensive benchmarking for Mask2Former models."""
    ...
    def _calculate_flops(self, model, config):
        """Calculate FLOPS for Mask2Former model."""
    ...
    def _measure_inference_time(self, model, config, num_runs=100, warmup_runs=10):
        """Measure inference time and memory usage for Mask2Former on current device."""
        model.eval()
        rgb, lidar = self._create_dummy_input(config, 'fusion')
        modality = config['CLI']['mode']
    ...
    def benchmark_config(self, config_path):
        """Benchmark a single Mask2Former configuration."""
        print(f"\n{'='*60}")
    ...
    def benchmark_all_configs(self):
        ...
        print(f"Starting Mask2Former Model Benchmarking")
        ...
        for config_path in self.config_paths:
            try:
                ...
                if backbone == 'mask2former':
                    ...
                    print(f"Skipping non-Mask2Former config {config_path} (backbone: {backbone})")
                    continue
            except Exception as e:
    ...
    def save_results(self, output_path=None):
        ...
        if output_path is None and self.config_paths:
            ...
            output_path = os.path.join(benchmark_dir, f'mask2former_benchmark_results_{timestamp}.json')
...
def main():
    parser = argparse.ArgumentParser(description='Benchmark Mask2Former models')
    ...
    for device in devices_to_test:
        ...
        benchmarker = Mask2FormerBenchmarker(
    ...
    combined_benchmarker = Mask2FormerBenchmarker(config_files, devices_to_test[0], args.epoch_file)
