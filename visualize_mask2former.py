...
# -*- coding: utf-8 -*-
"""
Visualization script for Mask2Former Fusion models.
...
def process_images(model, data_loader, visualizer, image_paths, dataroot,
    ...
    for idx, path in enumerate(image_paths, 1):
        ...
        # Forward pass
        # Mask2Former returns (None, segmap, all_class_logits, all_masks)
        with torch.no_grad():
            try:
                model_outputs = model(rgb_input, lidar_input, modality)
                pred_logits = model_outputs[1]  # segmap: [B, num_classes, H, W]
...
def main():
    parser = argparse.ArgumentParser(description='Visualize Mask2Former Fusion Model Predictions')
